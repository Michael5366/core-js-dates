...
function getNextFriday(date) {
  const dataDayIndex = date.getDay();
  const fridayIndex = 5;

  const nextFridayIndex = (fridayIndex - dataDayIndex + 7) % 7 || 7;

  const nextFriday = new Date(date);
  nextFriday.setDate(date.getDate() + nextFridayIndex);
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (endTime - startTime) / oneDayTime + 1;
}
...
function formatDate(date) {
  ...
  const minSet = min.toString().padStart(2, 0);
  const secSet = sec.toString().padStart(2, 0);

  return `${month}/${day}/${year}, ${hour}:${minSet}:${secSet} ${amOrPm}`;
}
...
function getNextFridayThe13th(date) {
  ...
  let nextDate = new Date(year, month, 13);
